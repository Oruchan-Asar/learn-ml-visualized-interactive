...
import { USERS, ITEMS, train, predict, totalError, MISSING } from "@/lib/math-core/recommender-systems";
import { recordCheckpointAttempt } from "@/lib/mastery/store";
import { useCheckpointPassed } from "@/lib/mastery/useCheckpointPassed";
import styles from "../../part-2-classical-ml/gradient-descent-variants/DescentControls.module.css";

const CONCEPT_ID = "recommender-systems";

function predictedGrid(k: number): number[][] {
  const factors = train(k, 2000);
  return USERS.map((u) => ITEMS.map((i) => Math.round(predict(factors, u, i) * 100) / 100));
}
...
export function IntuitionDemo() {
  ...
  return (
    <>
      ...
      <KernelHeatmap kernel={grid} width={220} label={`rows = users, cols = items — total error ${totalError(factors).toFixed(2)}`} />
    </>
  );
}
...
export function RecommenderCheckpoint() {
  const [k, setK] = useState(1);
  const [hasInteracted, setHasInteracted] = useState(false);
  const everPassed = useCheckpointPassed(CONCEPT_ID);

  const factors = train(k, 2000);
  const err = totalError(factors);
