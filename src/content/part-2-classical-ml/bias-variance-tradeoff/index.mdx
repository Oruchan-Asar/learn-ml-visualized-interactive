...
<WorkedExample>
  The true curve here is exactly quadratic. A degree-0 fit (just the mean) has bias² $= 3.3$ across the six samples — it structurally cannot bend, so it's consistently wrong by roughly the same amount every time; its variance is a tiny $0.003$. A degree-4 fit, with exactly enough parameters to hit all five points exactly, drives bias² to $0$ — but its variance explodes to $0.173$, nearly 50 times higher. Right in between, degree 2 (matching the true shape) reaches bias² $\approx 0$ *and* a modest variance of just $0.046$ — the lowest total error of any degree tried.
</WorkedExample>
