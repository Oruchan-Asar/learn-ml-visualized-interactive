...
<WorkedExample>
  ...
  <Steps>
    <Step label="Degree 0 — too stiff">
      ...
      - bias² $= 3.3$ — it structurally cannot bend, so it's consistently wrong by roughly the same amount every time
      - variance is a tiny $0.003$
    </Step>
    <Step label="Degree 4 — too flexible">
      With exactly enough parameters to hit all five points exactly:
      - bias² drops to $0$
      - variance explodes to $0.173$, nearly 50 times higher
    </Step>
    <Step label="Degree 2 — matches the true shape">
      - Bias² $\approx 0$
      - a modest variance of just $0.046$
