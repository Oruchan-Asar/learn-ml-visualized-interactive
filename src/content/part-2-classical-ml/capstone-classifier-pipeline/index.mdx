...
<WorkedExample>
  AdaBoost's very first stump splits at $x=9.5$ — the identical threshold Chapter 8's depth-1 tree finds on its own. That's not a coincidence: both are searching for the single split that best separates the data, and there's only one true boundary here. The remaining four boosting rounds add nothing further, because the first stump already reached every validation point boosting could hope to fix.
</WorkedExample>
