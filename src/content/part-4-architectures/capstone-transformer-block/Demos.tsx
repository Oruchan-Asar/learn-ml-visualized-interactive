...
const CONCEPT_ID = "capstone-transformer-block";
...
/** Both toy attention heads, shown as small weight matrices — rows/cols are x1, x2 in that order. */
function Heads() {
  return (
    <div className={localStyles.heads}>
      <KernelHeatmap kernel={HEAD_A_MATRIX} label={HEAD_A_LABEL} width={110} />
      <KernelHeatmap kernel={HEAD_B_MATRIX} label={HEAD_B_LABEL} width={110} />
    </div>
  );
}
...
function Pipeline({ bias }: { bias: number }) {
  ...
  return (
    <div className={localStyles.pipeline}>
      ...
      <div className={localStyles.stageLabel}>multi-head attention output (Head A ⊕ Head B)</div>
...
/** Intuition beat: both heads, then the full block pipeline end to end, for two tokens. */
export function IntuitionDemo() {
  return (
    <>
      <Heads />
      <Pipeline bias={DEFAULT_BIAS} />
    </>
  );
}
...
export function CapstoneTransformerBlockCheckpoint() {
  const [bias, setBias] = useState(DEFAULT_BIAS);
