...
const CONCEPT_ID = "what-is-a-gradient";
const TARGET_GRADIENT = 0;
const TOLERANCE = 0.2;
...
export function PlayDemo() {
  ...
  return (
    <CurvePlayground
      ...
      value={x}
      onChange={setX}
      readout={`∇f(${x.toFixed(2)}) = 2(${x.toFixed(2)}) − 4 = ${g.toFixed(2)}`}
    />
  );
}

/** Checkpoint: drag until the gradient reads ~0 — i.e. find the minimum. */
export function GradientCheckpoint() {
  ...
  const g = gradient(x);
  const passed = withinTolerance(g, TARGET_GRADIENT, TOLERANCE);
  ...
  return (
    <CheckpointFrame
      instructions={
        <>
          Drag the point until the gradient reads (approximately) <strong>zero</strong> — that&rsquo;s the
    ...
    >
      <CurvePlayground
        ...
        passed={passed}
        readout={`∇f(${x.toFixed(2)}) = ${g.toFixed(2)}`}
      />
    </CheckpointFrame>
  );
}
