 ...
 */
export function CheckpointFrame({ instructions, passed, hasInteracted, idleLabel = "Try it", children }: CheckpointFrameProps) {
  return (
    <section className={styles.beat}>
      ...
      <div className={passed ? frameStyles.framePassed : frameStyles.frame}>
        ...
        <div className={passed ? frameStyles.statusPassed : frameStyles.status}>
          {passed ? "✓ Passed — mastery recorded" : hasInteracted ? "Not quite — adjust and try again" : idleLabel}
        </div>
      </div>
    </section>
  );
